...
function openSubmission(id) {
  ...
  scopeField.insertAdjacentHTML("afterend", `<div class="field" style="margin-top:12px"><label>Ciclos ou fases opcionais — uma por linha</label><textarea class="control" id="proposal-cycles" placeholder="Ex.: Ciclo 1 | Governança de pessoas | Estruturar papéis, ritos e decisões | Meses 1 e 2">${escapeHtml((savedCalc.cycles || []).map((cycle)=>[cycle.title,cycle.focus,cycle.objective,cycle.duration].filter(Boolean).join(" | ")).join("\n"))}</textarea><small class="field-help">Formato: Nome do ciclo | Foco | Objetivo | Duração. Adicione quantos ciclos forem necessários; o PDF cria páginas extras.</small></div><div class="field" style="margin-top:12px"><label>Cadência e formato — um item por linha</label><textarea class="control" id="proposal-cadence">${escapeHtml((savedCalc.cadence || cadenceDefaults).join("\n"))}</textarea><small class="field-help">Reuniões, checkpoints, canal e presencialidade. Não repita horas ou duração contratual.</small></div><div class="calc-grid" style="margin-top:12px"><div class="field"><label>Responsabilidades da CALI — uma por linha</label><textarea class="control" id="proposal-cali-responsibilities">${escapeHtml((savedCalc.caliResponsibilities || ["Conduzir as análises, encontros e devolutivas previstos no escopo.","Organizar decisões, responsáveis e próximos movimentos."]).join("\n"))}</textarea></div><div class="field"><label>Responsabilidades do cliente — uma por linha</label><textarea class="control" id="proposal-client-responsibilities">${escapeHtml((savedCalc.clientResponsibilities || ["Disponibilizar dados, pessoas e aprovações necessários ao trabalho.","Designar responsáveis internos e participar dos checkpoints acordados."]).join("\n"))}</textarea></div></div><div class="field" style="margin-top:12px"><label>Prioridades para ciclos posteriores — uma por linha</label><textarea class="control" id="proposal-roadmap-items">${escapeHtml((savedCalc.roadmapItems || []).join("\n"))}</textarea><small class="field-help">Esses itens aparecem no roadmap, não como entregas contratadas agora.</small></div><div class="field" style="margin-top:12px"><label>Fora do escopo — um item por linha</label><textarea class="control" id="proposal-out-of-scope">${escapeHtml((savedCalc.outOfScope || profile.outOfScope).join("\n"))}</textarea><small class="field-help">Existe um padrão seguro para cada serviço, mas todo o conteúdo é editável conforme a realidade da proposta.</small></div><h4 class="drawer-subheading">4 · Condições comerciais e fechamento</h4><div class="bonus-builder"><div class="field"><label>Bônus de contratação</label><select class="control" id="bonus-choice">${bonusOptions.map(([code,label])=>`<option value="${code}" ${savedBonus.code===code?"selected":""}>${escapeHtml(label)}</option>`).join("")}</select></div><div id="bonus-fields" class="calc-grid"><div class="field"><label>Nome do bônus</label><input class="control" id="bonus-title" value="${escapeHtml(savedBonus.title || "")}" maxlength="140"></div><div class="field"><label>O que o cliente receberá</label><textarea class="control" id="bonus-description" maxlength="500">${escapeHtml(savedBonus.description || "")}</textarea></div></div><small class="field-help">O bônus só aparece no PDF quando você selecionar e confirmar o conteúdo.</small></div>`);
...
async function saveProposal() {
  ...
  const calculatorData = { ...calculation, editorialVersion:2, monthlyHours:Number(document.getElementById("monthly-hours")?.value || 0), discountType:document.getElementById("discount-type").value, discountDescription:document.getElementById("discount-description").value.trim(), budgetStrategy:document.getElementById("budget-strategy")?.value || "manter", investmentAnswer:investmentContextFor(service, selected.answers || {}), contextSummary:document.getElementById("proposal-context").value.trim(), painPoints:lines("proposal-pains"), executiveReading:document.getElementById("proposal-reading").value.trim(), mapaPeople, whyNow:document.getElementById("proposal-why-now").value.trim(), cycleObjective:document.getElementById("proposal-cycle-objective").value.trim(), expectedResults:lines("proposal-results"), advantages, cycles, cadence:lines("proposal-cadence"), caliResponsibilities:lines("proposal-cali-responsibilities"), clientResponsibilities:lines("proposal-client-responsibilities"), roadmapItems:lines("proposal-roadmap-items"), outOfScope:lines("proposal-out-of-scope"), bonus, payment };
