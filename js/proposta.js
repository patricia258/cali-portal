...
const cycleChunks=[];
for(let i=0;i<cycles.length;i+=6) cycleChunks.push(cycles.slice(i,i+6));
const roadmapHtml=(compact=false)=>`<aside class="proposal-roadmap-note${compact?" proposal-cycle-roadmap":""}"><strong>Roadmap posterior</strong><p>${escapeHtml(roadmapNote)}</p>${roadmapItems.length?linesHtml(roadmapItems,"proposal-roadmap-list"):""}</aside>`;
...
scopeChunks.forEach((chunk,index)=>addPage("Entrega",`<div class="proposal-kicker">Escopo contratado</div><h1>${index?"Continuação do escopo":"Escopo incluído"}</h1><p class="proposal-page-intro">As entregas abaixo seguem exatamente a ordem definida para esta proposta.</p>${scopeChunkHtml(chunk)}${index===scopeChunks.length-1?`<section class="proposal-section proposal-working"><h2>Cadência</h2>${linesHtml(cadence,"proposal-simple-list")}</section><section class="proposal-section"><h2>Responsabilidades</h2><div class="proposal-responsibility-grid"><div><span>A CALI conduz</span>${linesHtml(caliResponsibilities,"proposal-simple-list")}</div><div><span>O cliente fornece</span>${linesHtml(clientResponsibilities,"proposal-simple-list")}</div></div></section>${cycles.length?"":roadmapHtml()}`:""}`,`proposal-delivery-page`));
cycleChunks.forEach((chunk,index)=>addPage("Ciclos",`<div class="proposal-kicker">Sequência de implantação</div><h1>${index?"Continuação dos ciclos":"Ciclos e fases de implantação"}</h1><div class="proposal-cycle-list">${chunk.map((cycle,cycleIndex)=>`<section><span>${String(index*6+cycleIndex+1).padStart(2,"0")}</span><div><div class="proposal-cycle-heading"><h2>${escapeHtml(cycle.title)}</h2>${cycle.duration?`<small>${escapeHtml(cycle.duration)}</small>`:""}</div>${cycle.focus?`<strong>${escapeHtml(cycle.focus)}</strong>`:""}${cycle.objective?`<p>${escapeHtml(cycle.objective)}</p>`:""}</div></section>`).join("")}</div>${index===cycleChunks.length-1?roadmapHtml(true):""}`,`proposal-cycles-page`));
