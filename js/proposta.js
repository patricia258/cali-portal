...
import { SERVICES, flattenFields, trainingNeedsTravel } from "/js/services.js";
...
const normalizeCycle = (cycle) => {
  const structured = {
    title:String(cycle?.title || "").trim(),
    focus:String(cycle?.focus || "").trim(),
    objective:String(cycle?.objective || "").trim(),
    duration:String(cycle?.duration || "").trim(),
  };
  if (structured.focus || structured.objective || structured.duration) return structured;
  const raw = structured.title.replace(/^\s*[-–—]\s*/, "");
...
const roadmapHtml=()=>`<aside class="proposal-roadmap-note proposal-cycle-roadmap"><strong>Roadmap posterior</strong><p>${escapeHtml(roadmapNote)}</p>${roadmapItems.length?linesHtml(roadmapItems,"proposal-roadmap-list"):""}</aside>`;
const outOfScopeHtml=()=>`<section class="proposal-section proposal-cycle-exclusions"><h2>Fora do escopo</h2>${linesHtml(outOfScope,"proposal-out-list")}</section>`;
...
if (isTraining) {
  ...
  const remoteTraining = String(answers.formato || "") === "online";
  const travelRequired = trainingNeedsTravel(answers) || Boolean(answers.deslocamento_necessario);
  const trainingLocation = String(answers.local_execucao || "").trim();
  const trainingAddress = String(answers.local_endereco_completo || "").trim();
  const travelEstimate = Number(calc.trainingTravelEstimate || 0);
  const travelEstimateLabel = travelEstimate > 0 ? currency(travelEstimate) : "A confirmar antes da contratação";
  const logisticsNote = remoteTraining
    ? "Realização via Google Meet. A sessão poderá contar com gravação e transcrição para registro, mediante ciência prévia dos participantes."
    : travelRequired
      ? `Realização presencial em ${trainingLocation || "local informado pelo cliente"}. Passagens, hospedagem, alimentação e deslocamentos locais ficam por conta da empresa contratante e não estão incluídos no investimento da CALI. Estimativa atual: ${travelEstimateLabel}.`
      : `Realização presencial em ${trainingLocation || "Curitiba / Região Metropolitana"}${trainingAddress ? `, no endereço ${trainingAddress}` : ""}.`;
  const travelCommercialHtml = travelRequired ? `<aside class="proposal-bonus"><span>Despesas logísticas</span><div><strong>${escapeHtml(travelEstimateLabel)}</strong><p>Passagens, hospedagem, alimentação e deslocamentos locais são de responsabilidade da contratante e não integram o investimento da CALI.</p></div></aside>` : "";
