...
export const SERVICES = {
  ...
  "mentoria-rh": {
    ...
    sections: [
      ...
      { title: "Formato do ciclo", fields: [
        { id: "modalidade", label: "Modalidade", type: "radio", required: true, options: [option("individual", "Individual"), option("grupo", "Grupo da mesma empresa")] },
        { id: "participantes", label: "Número de participantes", type: "number", min: 1, max: 5, value: 1, span: 4 },
        { id: "frequencia", label: "Frequência", type: "select", required: true, span: 4, options: [option("quinzenal", "Quinzenal"), option("mensal", "Mensal")] },
        { id: "duracao_sessao", label: "Duração preferida", type: "select", required: true, span: 4, options: [option("60", "60 minutos"), option("90", "90 minutos")] },
        { id: "suporte", label: "Suporte entre sessões", type: "select", required: true, span: 6, options: [option("essencial", "Somente materiais e tarefas"), option("mensagens", "Mensagens para dúvidas pontuais"), option("proximo", "Acompanhamento mais próximo")] },
        { id: "prazo_inicio", label: "Quando gostaria de iniciar?", type: "select", required: true, span: 6, options: [option("imediato", "O quanto antes"), option("30", "Em até 30 dias"), option("planejamento", "Estou planejando")] },
      ]},
    ],
    alerts(a) { const x=[]; if (Number(a.participantes)>5) x.push({level:"high",text:"A mentoria em grupo é limitada a 5 participantes da mesma empresa."}); if(a.suporte==="proximo") x.push({level:"medium",text:"Prever carga adicional de suporte entre sessões."}); return x; },
  },
  "diagnostico-executivo": {
    ...
    alerts(a){const x=[]; if(Number(a.entrevistas)>10)x.push({level:"medium",text:"Mais de 10 entrevistas: revisar carga e cronograma."}); if(a.documentos==="desorganizada")x.push({level:"high",text:"Aplicar acréscimo por organização documental."}); if(a.prazo_inicio==="imediato")x.push({level:"medium",text:"Confirmar disponibilidade mínima de quatro semanas."}); return x;},
  },
  "cultura-direcao": {
    ...
    alerts(a){const x=[]; if(Number(a.colaboradores)>200||Number(a.unidades)>3)x.push({level:"medium",text:"Revisar amostra, comunicação e logística por porte."}); if((a.contexto||[]).includes("ma"))x.push({level:"high",text:"Contexto de M&A exige trilha específica de integração cultural."}); return x;},
  },
  "shadowing-lideranca": {
    ...
    alerts(a){const x=[]; if(a.presencial==="remoto")x.push({level:"medium",text:"O presencial é recomendado por ser a essência do Shadowing."}); if(Number(a.lideres)>5)x.push({level:"high",text:"Mais de 5 líderes: dividir em ondas ou ampliar prazo."}); return x;},
  },
  treinamentos: {
  ...
  "marca-empregadora": {
    ...
    alerts(a){const x=[]; if(a.reviews==="negativas")x.push({level:"high",text:"Prever frente de reputação e resposta a passivos de percepção."}); if(a.modelo_contratacao==="recorrente"&&(a.ativos||[]).includes("evp"))x.push({level:"medium",text:"Separar fase inicial de definição do EVP da sustentação mensal."}); return x;},
  },
};
